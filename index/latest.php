...
$countQuery = $DB->query("SELECT COUNT(*) as total 
                          ...
                          WHERE STR_TO_DATE(c.addtime, '%Y-%m-%d %H:%i:%s') >= STR_TO_DATE('$one_week_ago', '%Y-%m-%d %H:%i:%s')");
...
$query = $DB->query("SELECT c.cid, c.fenlei as category_id, c.name as course_name, c.addtime, f.name as category_name 
                     FROM qingka_wangke_class c
                     JOIN qingka_wangke_fenlei f ON c.fenlei = f.id
                     WHERE STR_TO_DATE(c.addtime, '%Y-%m-%d %H:%i:%s') >= STR_TO_DATE('$one_week_ago', '%Y-%m-%d %H:%i:%s') 
...
<div class="layui-container" style="padding: 20px;" id="sx">
    <div class="layui-row" style="margin-bottom: 15px;">
        <div class="layui-card">
            <div class="panel-heading font-bold layui-bg-blue" style="box-shadow: 3px 3px 8px #d1d9e6, -3px -3px 8px #d1d9e6;border-radius: 7px;">最新上架【7天内】</div>
            <div class="layui-card-body">
                ...
                <table class="layui-table" style="table-layout: fixed; width: 100%;">
                    ...
                    <tbody>
                        ...
                        if ($DB->affected() == 0) {
                            echo "<tr><td colspan='5'>最近一周内没有上新课程。</td></tr>"; 
                        }
                        ?>
                    </tbody>
                </table>
